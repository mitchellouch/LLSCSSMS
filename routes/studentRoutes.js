...
router.post("/info/:studentId", async (req, res, next) => {
  ...
  var unsetObj = {};
  if (data.eaInfo === undefined) unsetObj.eaInfo = "";
  if (data.asInfo === undefined) unsetObj.asInfo = "";
  if (data.faInfo === undefined) unsetObj.faInfo = "";

  //console.log(unsetObj);

  var student = await Student.findOneAndUpdate(
    { saitId: req.params.studentId },
    { $unset: unsetObj, $set: data },
    { new: true }
  ).catch((error) => {
    console.log(error);
    res.sendStatus(400);
...
function buildDataObj(payload) {
  ...
  if (payload.studentServiceType && payload.studentServiceType.includes("EA")) {
    ...
    if (payload.academicStatus)
      data.eaInfo.academicStatus = payload.academicStatus;
  ...
  if (payload.studentServiceType && payload.studentServiceType.includes("AS")) {
    ...
    if (payload.citizenshipStatus)
      data.asInfo.citizenshipStatus = payload.citizenshipStatus;
    ...
    if (payload.secondaryCode)
      data.asInfo.secondaryCode = payload.secondaryCode;
    ...
    if (payload.emerg_relationship)
      data.asInfo.emergencyContact.relationship = payload.emerg_relationship;
    if (payload.emerg_fullName)
      data.asInfo.emergencyContact.fullName = payload.emerg_fullName;
    if (payload.emerg_phone)
      data.asInfo.emergencyContact.phone = payload.emerg_phone;
  }
  if (payload.studentServiceType && payload.studentServiceType.includes("FA")) {
    ...
    if (payload.hasIncomeSupport)
      data.faInfo.hasIncomeSupport = payload.hasIncomeSupport ? true : false;
    if (payload.hasEiClaim)
      data.faInfo.hasEiClaim = payload.hasEiClaim ? true : false;
    if (payload.hasReducedCrsLoad)
      data.faInfo.hasReducedCrsLoad = payload.hasReducedCrsLoad ? true : false;
    if (payload.isFundedEsl)
      data.faInfo.isFundedEsl = payload.isFundedEsl ? true : false;
    if (payload.isFundedEsl)
      data.faInfo.eslFundedMonths = payload.eslFundedMonths;
    if (payload.isFundedAu)
      data.faInfo.isFundedAu = payload.isFundedAu ? true : false;
